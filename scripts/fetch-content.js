...
async function main() {
  try {
    ...
    console.log(
      "📁 Copying content to /content directory (directories only)..."
    );
    ...
    console.log(
      "🏷️  Tagging uses built-in regex patterns, no definitions.yaml needed"
    );
    ...
    try {
      ...
      } = require("../src/lib/server-content.js");
      ...
      console.error(
        "⚠️  Warning: Failed to generate static content data:",
        buildError.message
      );
      console.log(
        "Content fetched successfully, but build-time optimization failed."
      );
    }
  } catch (error) {
